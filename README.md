...
The Booru map holds submaps of information(credentials and default tags) for the boorus we connect to, this allows you to increase operability depending on the booru. If you specify a `"default_tags" : []`, you'll be able to require or blacklist certain tags in booru queries. Some apis need credentials to work at all. The names of the subarrays can be found in Image.cs inside the Image.Board.Get function, the properties needed in these arrays are "login" and either "api_key" or "password_hash", the booru's site should tell you what should go here.
